<!-- Banner -->

<p align="center"> <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=28&duration=3200&pause=900&color=8B3DFF&center=true&vCenter=true&width=900&lines=Hey+there!+I'm+Piyush+Baniya+👋;Flutter+Developer+%26+Builder;Building+VoraTube+🎵+%26+VoraFind+🔎;From+Nepal+🇳🇵" /> </p>

<p align="center"> <b>Developer • Flutter • Mobile Apps • UI/UX • Open Source</b> </p>

👋 About Me

I'm Piyush Baniya, a developer from Nepal 🇳🇵 who enjoys turning ideas into polished, useful products.

My current focus is Flutter development, building cross-platform mobile applications with a strong emphasis on performance, clean architecture, privacy, and smooth user experiences.

I've explored multiple areas of development over the years, including Kotlin/Android, React, Node.js, Python, Django, and the MERN stack. These experiences help me approach mobile development from both a frontend and full-stack perspective.

I'm especially interested in building products from the ground up — from the initial idea and architecture to implementation, UI/UX, testing, and Play Store release.

When I'm not coding, I create gaming and tech content on YouTube and experiment with new ideas.

🚀 What I'm Building <br>
🎵 VoraTube

A modern local music player for Android & iOS built with Flutter.

VoraTube focuses on playing music that is already stored on the user's device, with a smooth, modern dark/AMOLED interface and features designed around a fast and enjoyable listening experience.

Tech: Flutter • Dart • Riverpod • Drift • just_audio • audio_service

<a href="https://voratube.vercel.app/" target="_blank" style="text-decoration: None">🔗 View VoraTube →</a>

🛠️ Tech Stack
📱 Mobile Development

<p align="left"> <img src="https://skillicons.dev/icons?i=flutter,dart,kotlin,androidstudio" /> </p>

🌐 Web & Full Stack

<p align="left"> <img src="https://skillicons.dev/icons?i=html,css,javascript,react,nodejs,express,mongodb,tailwind,bootstrap" /> </p>

🐍 Other Technologies

<p align="left"> <img src="https://skillicons.dev/icons?i=python,django,postgres" /> </p>

🧰 Tools

<p align="left"> <img src="https://skillicons.dev/icons?i=git,github,vscode,androidstudio,vercel,figma,photoshop" /> </p>

🧠 Currently Exploring <br>
🚀 Advanced Flutter development <br>
🏗️ Clean & scalable application architecture <br>
📱 Native Android integration with Flutter <br>
🗃️ Local databases & offline-first applications <br>
🧠 On-device AI & semantic search <br>
⚡ Performance optimization <br>
🎨 Modern UI/UX and animations <br>
🔐 Privacy-focused application design <br>
📦 App deployment & Play Store development <br>
📂 Featured Projects <br> <br>
Project	Description	Focus <br>
🎵 VoraTube	Local music player for Android & iOS	Flutter • Audio • Mobile <br>
🎯 My Development Philosophy 

Build things that are useful, fast, private, and enjoyable to use.

I don't just want to write code — I enjoy taking an idea and turning it into an actual product.

That means caring about:

Idea → Architecture → UI/UX → Development → Testing → Performance → Release

📺 Content & YouTube

Outside development, I create gaming and tech-related content on YouTube.

🔗 Connect With Me

<p align="left"> <a href="https://linkedin.com/in/piyush-das-baniya" target="_blank"> <img src="https://skillicons.dev/icons?i=linkedin" width="42" /> </a> <a href="https://instagram.com/piyush_baniya1" target="_blank"> <img src="https://skillicons.dev/icons?i=instagram" width="42" /> </a>

📧 Email: baniyapiyush0810@gmail.com

🌐 GitHub: github.com/piyush-baniya

👀 Profile Views →
<p align="left"> <img src="https://komarev.com/ghpvc/?username=piyush-baniya&label=Visitors&color=8B3DFF&style=flat" /> </p>

<p align="center"> <i>Building ideas into products, one commit at a time. 🚀</i> </p>
